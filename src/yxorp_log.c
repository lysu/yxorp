...
void log_reopen(void) {
    ...
    if (l->fd != STDERR_FILENO) {
        ...
        if (l->fd < 0) {
            log_stderr_safe("reopening log file '%s' failed, ignored: %s",
...
void _log(const char *file, int line, int panic, const char *fmt, ...) {
    ...
    len += yx_scnprintf(buf + len, size - len, "[%.*s] %s:%d ",
                        strlen(timestr) - 1, timestr, file, line);

    va_start(args, fmt);
    len += yx_vscnprintf(buf + len, size - len, fmt, args);
    va_end(args);
...
void _log_stderr(const char *fmt, ...) {
    ...
    va_start(args, fmt);
    len += yx_vscnprintf(buf, size, fmt, args);
    va_end(args);
...
void _log_hexdump(char *data, int datalen) {
    ...
    while (datalen != 0 && (len < size - 1)) {
        ...
        len += yx_scnprintf(buf + len, size - len, "%08x  ", off);
        ...
        for (i = 0; datalen != 0 && i < 16; data++, datalen--, i++) {
            ...
            str = (i == 7) ? "  " : " ";
            len += yx_scnprintf(buf + len, size - len, "%02x%s", c, str);
        }
        for (; i < 16; i++) {
            str = (i == 7) ? "  " : " ";
            len += yx_scnprintf(buf + len, size - len, "  %s", str);
        }

        data = save;
        datalen = savelen;

        len += yx_scnprintf(buf + len, size - len, "  |");

        for (i = 0; datalen != 0 && i < 16; data++, datalen--, i++) {
            c = (unsigned char)(isprint(*data) ? *data : '.');
            len += yx_scnprintf(buf + len, size - len, "%c", c);
        }
        len += yx_scnprintf(buf + len, size - len, "|\n");
...
void _log_safe(const char *fmt, ...) {
    ...
    len +=
        yx_safe_snprintf(buf + len, size - len, "[........................] ");

    va_start(args, fmt);
    len += yx_safe_vsnprintf(buf + len, size - len, fmt, args);
    va_end(args);
...

{
    ...
    n = yx_write(STDERR_FILENO, buf, len);
    if (n < 0) {
        l->nerror++;
    }
